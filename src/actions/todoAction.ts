...
export const getters = {
  // (state, getter) => cal state
  isCompletedFetch(state, getters) {
    console.log(`other getters ${getters}`);
    return state.status === FETCH_STATUS.SUCCESS;
  },
  searchedTodoItem: (state) => (keyword: string) => {
    if (!keyword) return state.todoList;
    return state.todoList.filter((todo) => todo.content.includes(keyword));
  },
};
